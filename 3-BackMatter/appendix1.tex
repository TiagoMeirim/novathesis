...
\chapter{Code And Performance Tables}
\label{app:appendix}

\section{Complete Case Studies}

\subsection{Tail Recursive Factorial}
\label{subsec:Tail_Recursive_Factorial}

\subsubsection*{OCaml + GOSPEL}
...
\subsubsection*{Generated CakeML}
...
\subsubsection*{Corrected CakeML 1st Version}
...
\subsubsection*{Corrected CakeML 2nd Version}
...
\label{subsec:Exception_Recursive_Search}
...
\subsubsection*{Corrected CakeML}
...
\label{subsec:Count_Even_For_A_List}
...
\label{subsec:Depth_search_tree}
...
module RedBlackTree
 ...
 | Node _ l k' v' r -> ((k = k') /\ (v = v')) \/ ((memt l k v) \/ (memt r k v))
  ...
  lemma memt_color : 
  forall l: tree, r: tree, k: key, k': key, v: value, v': value, c: color, c': color. 
  (memt (Node c l k v r) k' v') -> (memt (Node c' l k v r) k' v')
 ...
 forall k: key. forall v: value. (memt t k v) -> (x < k)
  ...
  lemma lt_tree_node : 
  forall x: key, y: key, v: value, l: tree, r: tree, c: color. 
  (lt_tree x l) -> ((lt_tree x r) -> ((y < x) -> (lt_tree x (Node c l y v r))))
  ...
  lemma lt_tree_not_in : 
  forall x: key, t: tree. 
  (lt_tree x t) -> (forall v: value. not memt t x v)

  lemma lt_tree_trans : 
  forall x: key, y: key. 
  (x < y) -> (forall t: tree. (lt_tree x t) -> (lt_tree y t))

  lemma gt_tree_not_in : 
  forall x: key, t: tree. 
  (gt_tree x t) -> (forall v: value. not memt t x v)

  lemma gt_tree_trans : 
  forall x: key, y: key. 
  (y < x) -> (forall t: tree. (gt_tree x t) -> (gt_tree y t))
 ...
 | Node _ l k _ r -> (bst l) /\ ((bst r) /\ ((lt_tree k l) /\ (gt_tree k r)))
  ...
  lemma bst_left : 
  forall k: key, v: value, l: tree, r: tree, c: color. 
  (bst (Node c l k v r)) -> (bst l)
  ...
  lemma bst_color : 
  forall c: color, c': color, k: key, v: value, l: tree, r: tree. 
  (bst (Node c l k v r)) -> (bst (Node c' l k v r))

  lemma rotate_left : 
  forall kx: key, ky: key, vx: value, vy: value, a: tree, b: tree, c: tree, 
 ...
 | Node Black l _ _ r -> 
  ...
  lemma rbtree_left : 
  forall x: key, v: value, l: tree, r: tree, c: color. 
  (exists n: int. rbtree n (Node c l x v r)) -> (exists n: int. rbtree n l)
 ...
 | Node Black l _ _ r -> (rbtree (n - 1) l) /\ (rbtree (n - 1) r)
  ...
  lemma rbtree_almost_rbtree_ex : 
  forall s: tree. (exists n: int. rbtree n s) -> (exists n: int. almost_rbtree n s)

  lemma almost_rbtree_rbtree_black : 
  forall x: key, v: value, l: tree, r: tree, n: int. 
  (almost_rbtree n (Node Black l x v r)) -> (rbtree n (Node Black l x v r))
  ...
  ensures { (bst result) /\ 
  ((forall n: int. (almost_rbtree n l) -> ((rbtree n r) -> (rbtree (n + 1) result))) /\ 
  (forall k': key, v': value. (memt result k' v') <-> (if k' = k then v' = v else (memt l k' v') \/ 
  (memt r k' v')))) } 
 = match l with
 ...
 = match r with
  ...
  ((is_not_red t) -> (rbtree n result)))) /\ 
  ((memt result k v) /\ 
  (forall k': key, v': value. (memt result k' v') <-> (if k' = k then v' = v else memt t k' v')))) } 
 = match t with
  | Leaf -> Node Red Leaf k v Leaf
  | Node Red l kqt vqt r -> 
    if k < kqt then Node Red (insert l k v) kqt vqt r 

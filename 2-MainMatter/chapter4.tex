...
%%insufficient in the description of the evaluation methodology
...
\section{CakeML Extraction}
\label{sec:CakeML_Extraction}
...
A small modification \footnote{Modified \cameleer code: \link{https://github.com/TiagoMeirim/cameleer/blob/master/bin/cli.ml}}
to the file \texttt{/bin/cli.ml} of \cameleer 's source code
...
\section{Study of the Initial Extraction Mechanism}
\label{sec:Case_Studies}

For all the case studies mentioned below, the \ocaml + \gospel code was verified through \whythree automatic
...
our pipeline can function if small changes are applied to the extraction mechanism. The complete code can be seen in \ref{subsec:Tail_Recursive_Factorial}.
...
as well as handling exceptions. The complete code can be seen in \ref{subsec:Exception_Recursive_Search}.
...
code compilable and correct. The complete code can be seen in \ref{subsec:Count_Even_For_A_List}.
...
produces compilable \cml code. The complete code can be seen in \ref{subsec:Depth_search_tree}.
...
The complete code can be seen in \ref{subsec:Tree_Comparison}.
...
both syntactically correct and successfully compilable. The complete code can be seen in \ref{subsec:References}.

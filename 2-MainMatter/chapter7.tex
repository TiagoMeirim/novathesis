...
The propagation lemmas \inlinecode{lt\_left}, \inlinecode{lt\_right}, \inlinecode{gt\_left} and \inlinecode{gt\_right} says that
...
The values presented in the following tables \ref{subsec:Red_Black_Trees_Original_Whyml} and \ref{subsec:Red_Black_Trees_Translated_Whyml} show 
very similar values, making no significant changes in terms of prover efficiency. What changed was the provers while execution times remained relatively 
consistent, exhibiting no apparent fluctuations. It is to be expected since the specification had no major changes from the original after being
translated from \cml to \whyml. The only notable distinction is in Section \ref{sec:Verification_Guarantees_and_Trust_Boundaries} where the 
...
to a language from the ML family, only needing the communication with the \gospel AST in order to print the correct language for an 
intermideate language like \whyml.